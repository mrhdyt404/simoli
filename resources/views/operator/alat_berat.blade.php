...
<div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
    <div>
        <h5 class="fw-bold m-0 text-dark fs-17"><i class="feather-truck me-1.5 text-primary"></i>Kelola Unit Alat Berat</h5>
        <div class="text-muted fs-12">PKS {{ Auth::user()->pks ? Auth::user()->pks->nama : 'Unit' }}</div>
    </div>
    <button type="button" class="btn btn-primary btn-sm rounded-pill px-3 shadow-sm fw-bold fs-12 d-inline-flex align-items-center gap-1" data-bs-toggle="modal" data-bs-target="#modalAddAlatBerat">
        <i class="feather-plus"></i>Tambah Unit Baru
    </button>
</div>

<!-- Search & Filter Card -->
<div class="op-card mb-3 p-2.5">
    <form action="{{ route('operator.alat-berat.index') }}" method="GET" class="row g-2">
        <div class="col-7 col-sm-8">
            <div class="input-group input-group-sm">
                ...
                <input type="text" name="search" class="form-control border-start-0 ps-0" placeholder="Cari Kode atau Nama Unit..." value="{{ request('search') }}">
            </div>
        </div>
...
<!-- Equipment List Cards Grid -->
<div class="row g-2">
    @forelse($alatBeratList as $ab)
        <div class="col-12 col-md-6">
            <div class="op-card mb-2 h-100 d-flex flex-column justify-content-between">
                <div class="op-card-header d-flex justify-content-between align-items-center py-2 px-3">
                    <div>
                        <span class="badge bg-primary-subtle text-primary border border-primary-subtle fw-bold me-1 fs-12">{{ $ab->kode_alat }}</span>
                        <span class="fw-bold text-dark fs-13">{{ $ab->nama_alat }}</span>
                    </div>
                    <!-- Quick Status Change Dropdown -->
                    <div class="dropdown">
                        <button class="btn btn-xs dropdown-toggle py-1 px-2.5 fw-bold rounded-pill 
                            ...
                            type="button" data-bs-toggle="dropdown" style="font-size: 0.72rem;">
                            {{ $ab->status }}
                        </button>
                        <ul class="dropdown-menu dropdown-menu-end shadow border-0 rounded-3">
                            <li><h6 class="dropdown-header fs-11 text-uppercase">Ubah Status Cepat</h6></li>
                            <li>
                                <form action="{{ route('operator.alat-berat.update-status', $ab->id) }}" method="POST">
                                    @csrf @method('PATCH')
                                    <input type="hidden" name="status" value="Operational">
                                    <button type="submit" class="dropdown-item text-success fs-12"><i class="feather-check-circle me-2"></i>Ready / Operational</button>
                                </form>
                            </li>
                            <li>
                                <form action="{{ route('operator.alat-berat.update-status', $ab->id) }}" method="POST">
                                    ...
                                    <button type="submit" class="dropdown-item text-secondary fs-12"><i class="feather-pause-circle me-2"></i>Standby / Cadangan</button>
                                </form>
                            </li>
                            <li>
                                <form action="{{ route('operator.alat-berat.update-status', $ab->id) }}" method="POST">
                                    ...
                                    <button type="submit" class="dropdown-item text-warning fs-12"><i class="feather-tool me-2"></i>Maintenance / Perbaikan</button>
                                </form>
                            </li>
                            <li>
                                <form action="{{ route('operator.alat-berat.update-status', $ab->id) }}" method="POST">
                                    ...
                                    <button type="submit" class="dropdown-item text-danger fs-12"><i class="feather-alert-triangle me-2"></i>Breakdown / Rusak</button>
                                </form>
                            </li>
                            <li>
                                <form action="{{ route('operator.alat-berat.update-status', $ab->id) }}" method="POST">
                                    ...
                                    <button type="submit" class="dropdown-item text-info fs-12"><i class="feather-repeat me-2"></i>Rolling / Dipinjam Kebun Lain</button>
                                </form>
                            </li>
                        </ul>
                    </div>
                ...
                <div class="op-card-body p-3">
                    <div class="row g-2 mb-2 text-muted fs-12">
                        <div class="col-6"><i class="feather-info me-1 text-primary"></i>Jenis: <strong class="text-dark">{{ $ab->jenis_alat }}</strong></div>
                        <div class="col-6"><i class="feather-cpu me-1 text-primary"></i>Merk: <strong class="text-dark">{{ $ab->merk_tipe ?? '-' }}</strong></div>
                        <div class="col-6"><i class="feather-calendar me-1 text-primary"></i>Tahun: <strong class="text-dark">{{ $ab->tahun_pengadaan ?? '-' }}</strong></div>
                    </div>

                    @if($ab->keterangan)
                        <div class="p-2 bg-light rounded-2 border text-muted fs-11 mb-2">
                            {{ $ab->keterangan }}
                        </div>
                    @endif

                    <div class="d-flex justify-content-end gap-2 pt-2 border-top">
                        <button type="button" class="btn btn-sm btn-outline-warning px-3 rounded-pill fs-12 fw-semibold" data-bs-toggle="modal" data-bs-target="#modalEditAlatBerat{{ $ab->id }}">
                        ...
                        <form action="{{ route('operator.alat-berat.destroy', $ab->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus unit alat berat ini?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-outline-danger px-3 rounded-pill fs-12 fw-semibold">
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content rounded-4 border-0 shadow">
                        <div class="modal-header bg-warning text-dark py-2.5 px-3">
                            <h6 class="modal-title fw-bold fs-14"><i class="feather-edit-2 me-1.5"></i>Edit Unit Alat Berat</h6>
                        ...
                        <form action="{{ route('operator.alat-berat.update', $ab->id) }}" method="POST">
                            ...
                            <div class="modal-footer p-2.5">
                                <button type="button" class="btn btn-light rounded-pill px-3" data-bs-dismiss="modal">Batal</button>
                                <button type="submit" class="btn btn-warning fw-bold rounded-pill px-3">Simpan Perubahan</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    @empty
        <div class="col-12 text-center p-4 bg-white rounded-3 border text-muted">
            <i class="feather-truck fs-2 d-block mb-2 text-muted opacity-50"></i>
            <div class="fs-13 fw-semibold">Belum ada unit alat berat</div>
            <div class="fs-11">Tekan tombol Tambah Unit Baru di atas untuk mendaftarkan alat.</div>
        </div>
    @endforelse
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            <div class="modal-header bg-primary text-white py-2.5 px-3">
                <h6 class="modal-title fw-bold fs-14"><i class="feather-plus-circle me-1.5"></i>Tambah Unit Alat Berat Baru</h6>
            ...
            <form action="{{ route('operator.alat-berat.store') }}" method="POST">
                ...
                <div class="modal-footer p-2.5">
                    <button type="button" class="btn btn-light rounded-pill px-3" data-bs-dismiss="modal">Batal</button>
                    <button type="submit" class="btn btn-primary fw-bold rounded-pill px-3">Tambah Alat Berat</button>
                </div>
            </form>
        </div>
    </div>
