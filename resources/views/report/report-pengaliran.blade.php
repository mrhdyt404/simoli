...
@section('title', 'Report Pengaliran')
...
<style>
    ...
    .table tfoot td { font-weight: 700; background: #ecfdf5; border-top: 2px solid #a7f3d0; font-size: 13px; }
    ...
    html.app-skin-dark .pks-group-header { background: linear-gradient(135deg, #064e3b 0%, #065f46 100%) !important; color: #6ee7b7 !important; border-color: #065f46 !important; }
    ...
    html.app-skin-dark .table tbody tr:hover { background-color: rgba(16,185,129,0.08) !important; }
    html.app-skin-dark .table tfoot td { background: #0f2922 !important; border-color: #065f46 !important; color: #6ee7b7 !important; }
    ...
    @media print {
        ...
        .row.mb-4 { display: none !important; } /* Hide stats section */
        ...
        .card-header { padding: 8px !important; margin: 0 !important; }
        .card-footer { padding: 6px 8px !important; margin: 0 !important; }
        ...
        /* PKS Group Header - inline dalam tabel */
        .pks-group-header { 
        ...
        /* Utilities cleanup */
        .rounded, .rounded-3, [class*="rounded"] { border-radius: 0 !important; }
        .mb-4, .mb-3 { margin-bottom: 0 !important; }
        .gap-3, .gap-2 { gap: 2px !important; }
        
        /* Remove color adjustments for dark mode */
        html.app-skin-dark .report-header-card,
        html.app-skin-dark .table thead th,
        html.app-skin-dark .table tfoot td,
    ...
    <table class="print-report-header-table">
        <tr>
            ...
            <td class="print-header-text-cell">
                <div class="print-header-title">Report Pengaliran SIMOLI</div>
...
<div class="print-only print-report-content">
    <p>Berikut report pengaliran SIMOLI untuk bulan {{ $namaBulan[(int)$bulan] }} {{ $tahun }}, yang memuat data per PKS:</p>
        ...
        <table class="print-report-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align:right;">{{ number_format($item->luas_area, 2) }}</td>
                    <td>{{ $item->rotasi ?? '-' }}</td>
            ...
            <tfoot>
                <tr class="print-subtotal-row">
                    ...
                    <td style="text-align:right;">{{ number_format($items->sum('luas_area'), 2) }}</td>
                    <td colspan="2"></td>
                </tr>
            </tfoot>
        </table>
...
<div class="card report-header-card shadow-sm mb-4">
    <div class="card-body p-4">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div>
                <h4 class="fw-bold mb-1"><i class="feather-droplet me-2"></i>Report Pengaliran</h4>
                <p class="text-muted-light mb-0 fs-13">
                    Laporan data pengaliran bulan {{ $namaBulan[(int)$bulan] }} {{ $tahun }}
                </p>
            </div>
            <div class="no-print">
...
<div class="row mb-4 no-print">
    ...
    <div class="col-md-3">
        <div class="card stat-card shadow-sm">
            <div class="card-body p-3">
                <div class="d-flex align-items-center gap-3">
                    <div class="stat-icon bg-soft-success text-success"><i class="feather-droplet"></i></div>
                    <div>
                        <h3 class="fw-bold mb-0">{{ number_format($summary['vol_dialirkan']) }}</h3>
    ...
    <div class="col-md-3">
        <div class="card stat-card shadow-sm">
            <div class="card-body p-3">
                <div class="d-flex align-items-center gap-3">
                    <div class="stat-icon bg-soft-info text-info"><i class="feather-layers"></i></div>
                    <div>
    ...
    <div class="col-md-3">
        <div class="card stat-card shadow-sm">
            <div class="card-body p-3">
                <div class="d-flex align-items-center gap-3">
                    <div class="stat-icon bg-soft-warning text-warning"><i class="feather-map"></i></div>
                    <div>
...
<div class="card filter-card shadow-sm mb-4 no-print">
    <div class="card-body py-3">
        <form action="{{ route('report-pengaliran') }}" method="GET">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-lg-5 col-md-12">
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('report-pengaliran') }}" class="btn btn-outline-secondary"><i class="feather-refresh-cw me-1"></i> Reset</a>
                    </div>
                </div>
            </div>
        </form>
...
<div class="card section-card shadow-sm">
    <div class="card-header py-3">
        <h6 class="mb-0 fw-bold"><i class="feather-droplet me-2 text-success"></i>Data Pengaliran — {{ $namaBulan[(int)$bulan] }} {{ $tahun }}</h6>
    </div>
    <div class="card-body p-0">
        @foreach($dataByPks as $pksName => $items)
        <div class="pks-group-header">
            <i class="feather-home me-1"></i> {{ $pksName }}
            <span class="badge bg-success ms-2">{{ $items->count() }} data</span>
        </div>
            ...
            <table class="table mb-0">
                <thead>
                    <tr>
                        ...
                        <th class="text-end">Luas Area</th>
                        <th>Rotasi</th>
                        <th>Keterangan</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end">{{ number_format($item->vol_limbah_dihasilkan) }}</td>
                        <td class="text-end">{{ number_format($item->vol_limbah_dialirkan) }}</td>
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="text-end">{{ number_format($items->sum('luas_area'), 2) }}</td>
                        <td colspan="2"></td>
                    </tr>
                </tfoot>
            </table>
    ...
    <div class="card-footer py-3">
        <div class="row text-center">
            ...
            <div class="col"><strong>Vol. Dihasilkan:</strong> {{ number_format($summary['vol_dihasilkan']) }} m³</div>
            <div class="col"><strong>Vol. Dialirkan:</strong> {{ number_format($summary['vol_dialirkan']) }} m³</div>
            <div class="col"><strong>Flat Bed:</strong> {{ number_format($summary['flat_bed']) }}</div>
            <div class="col"><strong>Luas Area:</strong> {{ number_format($summary['luas_area'], 2) }} Ha</div>
        </div>
    </div>
</div>
@else
<div class="card section-card shadow-sm">
    <div class="card-body">
        <div class="empty-report">
            ...
            <h5 class="text-muted mt-3">Tidak ada data pengaliran</h5>
