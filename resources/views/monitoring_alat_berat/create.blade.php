...
@section('page-description', 'Input Operasional & Jam Kerja (HM) Alat Berat Pengolahan Limbah')
...
<style>
    /* ================================================================
       MONITORING ALAT BERAT CREATE — PTPN GREEN THEME
       ================================================================ */
                        ...
                        <select name="alat_berat_id" class="form-select @error('alat_berat_id') is-invalid @enderror" required>
                            ...
                            @foreach($alatBeratList as $ab)
                                <option value="{{ $ab->id }}" {{ old('alat_berat_id') == $ab->id ? 'selected' : '' }}>
                                    [{{ $ab->kode_alat }}] {{ $ab->nama_alat }} ({{ $ab->jenis_alat }}){{ $ab->status === 'Standby' ? ' - [Standby]' : '' }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="gps-card">
                            ...
                            <div class="row g-2">
                                <div class="col-6">
                                    <input type="text" name="latitude_awal" id="latitude_awal" class="form-control form-control-sm" readonly placeholder="Lat Awal" value="{{ old('latitude_awal') }}">
                                    <input type="hidden" name="latitude" id="latitude" value="{{ old('latitude') }}">
                                </div>
                                <div class="col-6">
                                    <input type="text" name="longitude_awal" id="longitude_awal" class="form-control form-control-sm" readonly placeholder="Long Awal" value="{{ old('longitude_awal') }}">
                                    <input type="hidden" name="longitude" id="longitude" value="{{ old('longitude') }}">
                                </div>
                            </div>
                            <small id="gps_awal_status" style="font-size:11px;color:#6b7280;margin-top:6px;display:block;">Otomatis saat foto sebelum diupload</small>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="gps-card-akhir">
                            ...
                            <div class="row g-2">
                                <div class="col-6">
                                    <input type="text" name="latitude_akhir" id="latitude_akhir" class="form-control form-control-sm" readonly placeholder="Lat Akhir" value="{{ old('latitude_akhir') }}">
                                </div>
                                <div class="col-6">
                                    <input type="text" name="longitude_akhir" id="longitude_akhir" class="form-control form-control-sm" readonly placeholder="Long Akhir" value="{{ old('longitude_akhir') }}">
                                </div>
                            </div>
                            <small id="gps_akhir_status" style="font-size:11px;color:#6b7280;margin-top:6px;display:block;">Otomatis saat foto sesudah diupload</small>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-card">
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <input type="number" name="hm_awal" class="form-control @error('hm_awal') is-invalid @enderror" value="{{ old('hm_awal', 0) }}" min="0" step="any" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="hm_akhir" class="form-control @error('hm_akhir') is-invalid @enderror" value="{{ old('hm_akhir', 0) }}" min="0" step="any" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="bbm_liter" class="form-control @error('bbm_liter') is-invalid @enderror" value="{{ old('bbm_liter', 0) }}" min="0" step="any" required>
                ...
                <div class="row g-3 mt-2">
                    <div class="col-md-6">
                        ...
                        <input type="number" name="flat_bed" class="form-control" value="{{ old('flat_bed', 0) }}" min="0">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Jumlah Long Bed Dikerjakan</label>
                        <input type="number" name="long_bed" class="form-control" value="{{ old('long_bed', 0) }}" min="0">
                    </div>
                </div>
            </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="photo-upload-box" onclick="document.getElementById('foto_sesudah').click()">
                            ...
                            <input type="file" name="foto_sesudah" id="foto_sesudah" class="d-none" accept="image/*" onchange="previewImage(this, 'preview_sesudah', 'label_sesudah'); getGpsAkhir();">
                        </div>
            ...
            <div class="form-card">
                ...
                <div class="mb-3">
                    ...
                    <select name="kondisi_alat" class="form-select @error('kondisi_alat') is-invalid @enderror" required>
                        <option value="Normal" {{ old('kondisi_alat') == 'Normal' ? 'selected' : '' }}>Normal / Siap Pakai</option>
                        <option value="Perlu Perbaikan" {{ old('kondisi_alat') == 'Perlu Perbaikan' ? 'selected' : '' }}>Perlu Perbaikan Minor</option>
                        <option value="Breakdown" {{ old('kondisi_alat') == 'Breakdown' ? 'selected' : '' }}>Breakdown / Rusak Berat</option>
                    </select>
                ...
                <div>
                    ...
                    <textarea name="catatan" rows="4" class="form-control" placeholder="Tambahkan catatan khusus mengenai alat berat...">{{ old('catatan') }}</textarea>
                </div>
            </div>
    ...
    function getGpsAwal() {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(pos) {
                ...
                document.getElementById('gps_awal_status').textContent = '✓ GPS Awal berhasil didapatkan';
                document.getElementById('gps_awal_status').style.color = '#16a34a';
            }, function(err) {
    ...
    function getGpsAkhir() {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(pos) {
                ...
                document.getElementById('gps_akhir_status').textContent = '✓ GPS Akhir berhasil didapatkan';
                document.getElementById('gps_akhir_status').style.color = '#16a34a';
            }, function(err) {
                console.log(err.message);
            }, { enableHighAccuracy: true });
        }
